...
# ==========================================================
# 🎨 Advanced Plotting Utils (Fixed Colors)
# ==========================================================
...
def rasterize_labels(coords: np.ndarray, labels: pd.Series):
    ...
    cats = list(pd.Index(labels.astype(str).astype("category").cat.categories))
    code = pd.Categorical(labels.astype(str), categories=cats).codes
...
def plot_overlay_imshow(
    ...
    custom_colors: Optional[List[str]]=None # [Update] Accept custom colors
):
    K = len(cats)
    
    # Use custom colors if provided, otherwise auto
    ...
    ax.imshow(img_masked, cmap=cmap, origin=("upper" if origin_upper else "lower"),
    ...
    leg = ax.legend(
        handles=handles, loc=loc, bbox_to_anchor=anchor,
        borderaxespad=0.2, fontsize=10, frameon=True,
        title=f"Niches: {K}",
        labelspacing=0.3, handlelength=1.1, handletextpad=0.4, borderpad=0.4
    )
    fr = leg.get_frame()
...
def plot_facets_imshow(
    ...
    K = len(cats)
    if K == 0: return
    if ncols is None: ncols = int(math.ceil(math.sqrt(K)))
    ...
    for i, (cat, ax) in enumerate(zip(cats, axes)):
        ...
        cover_masked = np.ma.masked_invalid(cover); cover_cmap.set_bad(alpha=0.0)
        ax.imshow(cover_masked, cmap=cover_cmap, origin=("upper" if origin_upper else "lower"),
                  interpolation="nearest", extent=extent, vmin=0, vmax=1)
        layer = np.where(img == i, 1.0, np.nan)
        hl_cmap = ListedColormap([base_colors[i]]); hl_cmap.set_bad(alpha=0.0)
        ax.imshow(layer, cmap=hl_cmap, origin=("upper" if origin_upper else "lower"),
                  interpolation="nearest", extent=extent, vmin=0, vmax=1)
    ...
    if sample_title: fig.suptitle(f"{sample_title}", fontsize=16, y=0.995)
...
def get_gp_display_name(model, gp_name):
    try:
        ...
        if "gp_name" not in gp_summary.columns: gp_summary["gp_name"] = gp_summary.index
        row = gp_summary[gp_summary["gp_name"] == gp_name]
        if len(row) == 0: return gp_name
        sources = row["gp_source_genes"].values[0]
        targets = row["gp_target_genes"].values[0]
        if isinstance(sources, list): sources = " & ".join(sources)
        if isinstance(targets, list): targets = " & ".join(targets)
        if len(sources) > 15: sources = sources[:12] + ".."
        if len(targets) > 15: targets = targets[:12] + ".."
        return f"{sources} -> {targets}"
    except: return gp_name
...
if 'spatial' in model.adata.uns: del model.adata.uns['spatial']
...
# --- 3. Advanced Visualization (Unified Colors) ---
print(">>> [2/5] Plotting Advanced Spatial Maps (Rasterized)...")
samples = model.adata.obs[SAMPLE_KEY].unique().tolist()

# [Update] Get consistent colors from Scanpy dict
# create_new_color_dict returns: {'0': '#hex', '1': '#hex', ...}
latent_cluster_colors_dict = create_new_color_dict(adata=model.adata, cat_key=LATENT_CLUSTER_KEY)
...
for idx, s in enumerate(samples):
    ...
    # Rasterize returns cats list: ['0', '1', '2'...]
    img, cover, extent, cats = rasterize_labels(coords, labels)
    
    # [Update] Map cats to colors strictly
    # This ensures "Cluster 0" gets the same color in both plots
    ...
    plot_overlay_imshow(
        img, extent, cats, overlay_path, 
        sample_title=f"Niche Distribution ({s})",
        origin_upper=True,
        custom_colors=ordered_colors # Pass Scanpy colors
    )
    
    # 2. Facet Plot
    facet_path = os.path.join(figure_folder, f"niches_spatial_facet_{s}.png")
    plot_facets_imshow(
        img, cover, extent, cats, facet_path,
        sample_title=f"Separate Niches ({s})",
        origin_upper=True,
        custom_colors=ordered_colors # Pass Scanpy colors
    )

print(f"   ✅ Saved high-res spatial plots (Overlay & Facet)")
...
if "cluster" in model.adata.obs:
    ...
    # [UPDATE] Added rot=0 to make x-axis labels horizontal
    ...
    # [UPDATE] Added rot=0 here as well
...
if DIFFERENTIAL_GP_TEST_KEY in model.adata.uns:
    ...
    model.run_differential_gp_tests(cat_key=LATENT_CLUSTER_KEY, selected_cats=None, comparison_cats="rest", log_bayes_factor_thresh=2.3)
    results_df = model.adata.uns[DIFFERENTIAL_GP_TEST_KEY]

if "gp_name" not in results_df.columns:
    if "gene_program" in results_df.columns: results_df["gp_name"] = results_df["gene_program"]
    else: results_df["gp_name"] = results_df.index
...
print(">>> [5/5] Plotting Communication Networks (Enriched)...")
        ...
        try:
            ...
            if len(network_df) > 0:
                ...
                visualize_communication_gp_network(
                    ...
                    figsize=(10, 7),
                    cat_colors=latent_cluster_colors_dict, # Fixed: Use dict name
